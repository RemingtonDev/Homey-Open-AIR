...
const { delay, isValveOrCover } = require('../utils');
...
class ModernAdapter extends EventEmitter {
  ...
  _emitErrorSafely(error, details = {}) {
    this._recordDiagnostic('modern_adapter_error', {
      error,
      ...details,
    });
    ...
    this.logger.error('Modern adapter error with no listeners attached:', error);
  }
  ...
  async connect(waitForEntities = true) {
    ...
    this._recordDiagnostic('modern_connect_start', {
      waitForEntities,
    });
    ...
    return new Promise((resolve) => {
      ...
      const timeout = setTimeout(() => {
        ...
        this._recordDiagnostic('modern_connect_timeout', {
          waitForEntities,
        });
        resolve({ success: false, error: 'Connection timeout', errorType: 'timeout' });
      }, ESPHOME.CONNECTION_TIMEOUT_MS);

      // Disconnect & error handlers
      client.on('disconnect', (reason) => {
        this.logger.log(`[${version}] Disconnected: ${reason || 'unknown'}`);
        this._recordDiagnostic('modern_disconnected', { reason: reason || 'unknown' });
        if (!resolved) {
          resolved = true;
          clearTimeout(timeout);
          try { client.disconnect(); } catch (_) { /* ignore */ }
          const errorType = classifyConnectionError(reason || '');
          resolve({ success: false, error: reason || 'Disconnected', errorType });
          return;
        }
      ...
      client.on('error', (error) => {
        ...
        this._recordDiagnostic('modern_connect_error', { error });
        if (!resolved) {
          resolved = true;
          clearTimeout(timeout);
          try { client.disconnect(); } catch (_) { /* ignore */ }
          const errorType = classifyConnectionError(error);
          resolve({ success: false, error: error.message || String(error), errorType });
          return;
        }
      ...
      const stateTypes = ['fan', 'sensor', 'binary_sensor', 'switch', 'climate', 'light', 'cover', 'valve', 'number', 'select', 'text', 'button'];
      ...
      client.on('entities', async (entities) => {
        ...
        clearTimeout(timeout);
        this.client = client;
        this._recordDiagnostic('modern_connect_success', {
      ...
      client.on('connect', async (encrypted) => {
        ...
        if (resolved) {
          // Library reconnected internally — re-emit so wrapper can restore availability
          this.emit('connected');
          return;
        }
        resolved = true;
  ...
  sendValveCommand(entityKey, command) {
    ...
    const entityType = entityInfo?.type?.toLowerCase();

    if (entityType === 'cover') {
      this.logger.log(`Sending modern cover command to ${entityId}:`, command);
      this.client.sendCoverCommand(entityId, command);
    } else {
      this.logger.log(`Sending modern valve command to ${entityId}:`, command);
      this.client.sendValveCommand(entityId, command);
    }
  }

  /**
   * Send a button press command via the modern library.
   * @param {number} entityKey - Numeric entity key
   */
  sendButtonCommand(entityKey) {
    const entityId = this._keyToEntityId.get(entityKey);
    if (!entityId) {
      throw new Error(`No entity ID mapping for key ${entityKey}`);
    }
    this.logger.log(`Sending modern button command to ${entityId}`);
